...
describe('App shell', () => {
  it('mounts and shows the brand link after migration', async () => {
    ...
    // Match the brand link exactly to disambiguate from the same name in the Home heading.
    expect(await screen.findByRole('link', { name: /^grc suite\.v01$/i })).toBeInTheDocument();
  });

  it('exposes the primary navigation', async () => {
    render(<App />);
    await screen.findByRole('link', { name: /^grc suite\.v01$/i });
