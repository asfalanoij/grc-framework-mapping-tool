...
const FRAMEWORKS: readonly FrameworkRow[] = [
  { id: 'ISO 27001', name: 'ISO 27001:2022', path: '/iso27001', note: '118 controls — primary spine',
    itemIds: iso27001Controls.map((c) => c.id) },
  { id: 'NIST CSF 2.0', name: 'NIST CSF 2.0', path: '/nist-csf-2', note: '106 subcategories',
    itemIds: nistCsf2Subcategories.map((i) => i.id) },
  { id: 'SOC 2', name: 'SOC 2', path: '/soc2', note: '61 criteria',
    itemIds: soc2Criteria.map((i) => i.id) },
  { id: 'CIS v8', name: 'CIS v8', path: '/cis-v8', note: '153 safeguards',
    itemIds: cisV8Safeguards.map((i) => i.id) },
  { id: 'PCI DSS', name: 'PCI DSS 4.0.1', path: '/pci-dss', note: '12 requirements',
    itemIds: pciDssRequirements.map((g) => g.id) },
  { id: 'Cyber Essentials', name: 'Cyber Essentials', path: '/cyber-essentials', note: '5 themes',
    itemIds: cyberEssentialsThemes.map((g) => g.id) },
  { id: 'NIST 800-53', name: 'NIST 800-53', path: '/nist-800-53', note: '20 families',
    itemIds: nist80053Families.map((g) => g.id) },
  { id: 'NIS 2', name: 'NIS 2', path: '/nis2', note: '10 Art.21 measures',
    itemIds: nis2Measures.map((g) => g.id) },
  { id: 'ISO 22301', name: 'ISO 22301', path: '/iso22301', note: '7 clauses',
    itemIds: iso22301Clauses.map((g) => g.id) },
  { id: 'ISO 27017', name: 'ISO 27017', path: '/iso27017', note: '14 sections',
    itemIds: iso27017Sections.map((g) => g.id) },
  { id: 'NCSC CAF', name: 'NCSC CAF 4.0', path: '/ncsc-caf', note: '41 outcomes',
